...
def sample_weighted(p_dict):
    return np.random.choice(list(p_dict.values()))
...
class BaselineState(object):
    curve = lambda this, a: lambda x: 1
...
class FontState(object):
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        """
        if size is None:
            size = 20  # doesn't matter as we take the RATIO
        ...
        try:
            ...
            print("get_aspect_ratio {}".format(e))
            return 2.0
    ...
    def get_font_size(self, font, font_size_px):
        ...
        """
        m = self.font_model[font.name.encode()]
