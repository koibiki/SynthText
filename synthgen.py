...
from rect_uytils import has_interact
...
class TextRegions(object):
    ...
    @staticmethod
    def sample_grid_neighbours(mask, n_sample, step=3):
        """
...
class RendererV3(object):
    ...
    def _has_interact(self, rect, rects):
        rect = np.transpose(rect, (2, 1, 0))
        rects = np.transpose(rects, (2, 1, 0))

        for rect_word in rects:
            for w_rect in rect:
                if has_interact(w_rect, rect_word):
                    return True
        return False
            ...
            for idx in reg_range:
                ...
                try:
                    if self.max_time is None:
                        txt_render_res = self.place_text(img.copy(), place_masks[ireg],
                ...
                if txt_render_res is not None:
                    placed = True
                    img_render, text, bb, collision_mask = txt_render_res

                    word_box = self.char2wordBB(np.concatenate([bb], axis=2), ' '.join([text]))

                    if len(word_boxes) > 0:
                        if self._has_interact(word_box, word_boxes):
                            continue
                        word_boxes = np.concatenate([word_boxes, word_box], axis=2)
                    else:
                        word_boxes.append(word_box)
                        word_boxes = np.concatenate(word_boxes, axis=2)
